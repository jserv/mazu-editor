...
void clear_screen() { write(STDOUT_FILENO, "\x1b[2J", 4); }
...
int read_key() {
  ...
  while ((nread = read(STDIN_FILENO, &c, 1)) != 1) {
    if ((nread == -1) && (errno != EAGAIN)) panic("Error reading input");
  }
  if (c == '\x1b') {
    char seq[3];
...
bool is_part_of_number(int c) {
  return c == '.' || c == 'x' || c == 'a' || c == 'b' || c == 'c' || c == 'd' ||
         c == 'e' || c == 'f' || c == 'A' || c == 'X' || c == 'B' || c == 'C' ||
         c == 'D' || c == 'E' || c == 'F' || c == 'h' || c == 'H';
}
...
void highlight(editor_row *row) {
      ...
      for (j = 0; keywords[j]; j++) {
        ...
        if (!strncmp(&row->render[i], keywords[j], kw_len) &&
          ...
          memset(&row->highlight[i],
                 kw_2 ? KEYWORD_2 : (kw_3 ? KEYWORD_3 : KEYWORD_1), kw_len);
          i += kw_len;
          break;
        }
      }
  ...
  if (changed && row->idx + 1 < ec.num_rows) highlight(&ec.row[row->idx + 1]);
}
...
void cut() {
  ...
  if (ec.num_rows - ec.cursor_y > 0) highlight(&ec.row[ec.cursor_y]);
  if (ec.num_rows - ec.cursor_y > 1) highlight(&ec.row[ec.cursor_y + 1]);
  ec.cursor_x = ec.cursor_y == ec.num_rows ? 0 : ec.row[ec.cursor_y].size;
}

void paste() {
  if (!ec.copied_char_buffer) return;
...
void insert_char(int c) {
  if (ec.cursor_y == ec.num_rows) insert_row(ec.num_rows, "", 0);
...
char *rows_tostring(int *buf_len) {
  int total_len = 0;
  for (int j = 0; j < ec.num_rows; j++) total_len += ec.row[j].size + 1;
  *buf_len = total_len;
...
void open_file(char *file_name) {
  ...
  if (!file) panic("Failed to open the file");
  char *line = NULL;
  size_t line_cap = 0;
  ssize_t line_len;
...
void handle_sigwinch() {
  update_window_size();
  if (ec.cursor_y > ec.screen_rows) ec.cursor_y = ec.screen_rows - 1;
  if (ec.cursor_x > ec.screen_cols) ec.cursor_x = ec.screen_cols - 1;
  refresh_screen();
}
...
void process_key() {
  ...
  switch (c) {
    case '\r':
      newline();
      for (int i = 0; i < indent_level; i++) insert_char('\t');
      break;
    case CTRL_('q'):
      if (ec.modified && !prompt("File has been modified. Type 'yes' and enter "
    ...
    case CTRL_('s'):
      ...
      break;
    case CTRL_('x'):
      if (ec.cursor_y < ec.num_rows) cut();
      break;
    case CTRL_('c'):
      if (ec.cursor_y < ec.num_rows) copy(0);
      break;
    case CTRL_('v'):
      paste();
      break;
    ...
    case HOME_KEY:
      ...
      break;
    case END_KEY:
      if (ec.cursor_y < ec.num_rows) ec.cursor_x = ec.row[ec.cursor_y].size;
      break;
    case CTRL_('f'):
      search();
      break;
    ...
    case '{':
      ...
      break;
    case '}':
      if (ec.cursor_y == ec.num_rows) goto none;
      if ((ec.cursor_x == 0) && (ec.cursor_y == 0)) goto none;
      editor_row *row = &ec.row[ec.cursor_y];
